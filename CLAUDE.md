...
npm link               # optional: install globally so `multi-claude` resolves
...
- **`src/index.ts`** — CLI entry point using Commander.js. Defines all subcommands: `start`, `stop`, `list`, `attach`, `config-show`, `config-set`, `restart`, `rename`, `kill-all`, `models`. Each command handler orchestrates between config persistence and terminal operations.
...
- `startSession` is idempotent: it kills any existing tmux window with the same name before creating a new one. It builds the command, single-quote-escapes every argument via `shellQuote`, then uses `spawnSync` + `tmux send-keys -l` to type it into the pane. No shell interpolation happens in the orchestrator process. The stored PID is the tmux pane's shell PID (`#{pane_pid}`), not the claude process PID (which hasn't started yet).
...
- All tmux interaction uses `spawnSync` with argument arrays — no `execSync` or shell string interpolation. This prevents injection through session names.
- The `list` command uses ANSI color codes: green `▶ running`, red `■ stopped`. The `rename` command uses green for the `(running)` label.
